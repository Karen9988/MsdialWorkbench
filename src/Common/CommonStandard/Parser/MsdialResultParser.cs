...
{
    public sealed class MsDialResultParser
    {
        private MsDialResultParser() { }
        ...
        public static List<MoleculeMsReference> MsDialResultRecords(TextReader reader, out string error)
        {
            ...
            while (reader.Peek() > -1)
            {
                ...
                if (n != 31)
                {
                    messages.Add(string.Format("Msdial result parse failed as line column num not correct at line{0}", n));
                    continue;
                }

                var reference = new MoleculeMsReference() { ScanID = counter - 1 };
                reference.Name = lineArray[1].Trim('"');
                if (double.TryParse(lineArray[6], out double mz))
                {
                    //Console.WriteLine(string.Format("mz is {0}", mz));
                    if (mz < 0)
                    {
                ...
                if (double.TryParse(lineArray[4], out double rt)) {
                ...
                reference.AdductType = AdductIon.GetAdductIon(lineArray[10]);
                reference.Comment = "peakid=" + lineArray[0] + ", intensity=" + lineArray[8];
                
                reference.InChIKey = lineArray[17];
                //Console.WriteLine(string.Format("Inchikey:{0}", reference.InChIKey));

                reference.Formula = FormulaGenerator.Parser.FormulaStringParcer.OrganicElementsReader(lineArray[15]);
                ...
                //Console.WriteLine(string.Format("Formula:{0}", reference.Formula));

                reference.SMILES = lineArray[18];


                reference.Ontology = lineArray[16];

                reference.Spectrum = TextToSpectrumList(lineArray[30], ':', ' ');
                //Console.WriteLine(string.Format("SpectrumList:{0}", lineArray[30]));
        ...
        public static List<SpectrumPeak> TextToSpectrumList(string specText, char mzIntSep, char peakSep, double threshold = 0.0) {
            ...
            foreach (var spec in specArray) {
                ...
                if (mzInt.Length >= 2) {
                    ...
                    //Console.WriteLine(string.Format("mass:{0}, intensity:{1}", peak.Mass, peak.Intensity));
        ...
        public static List<MoleculeMsReference> MsDialResultRecords(string filePath, out string error)
        {
            ...
            {
                result = MsDialResultRecords(sr, out error);
            }
            return result;
        }
